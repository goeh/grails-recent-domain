...
class RecentDomainTagLib {
    ...
    def each = {attrs, body ->
        ...
        def list = type ? recentDomainService.getHistory(request, type) : recentDomainService.getHistory(request)
        if(list) {
            out << processList(list, attrs, body)
        }
    }

    private String processList(List list, Map attrs, Closure body) {
    ...
    def eachList = {attrs, body ->
        ...
        if(! list.isEmpty()) {
            ...
            out << processList(recentDomainService.convert(list, locale), attrs, body)
        }
    }
}
